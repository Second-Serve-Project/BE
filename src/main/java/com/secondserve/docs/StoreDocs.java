...
public interface StoreDocs {
    ...
    @GetMapping("/search")
    CompletableFuture<ApiResponse<List<StoreDto.Search>>> searchStore(@ModelAttribute SearchRequest searchRequest);


    @Operation(summary = "매장 세부 정보", description = "특정 매장의 세부 정보를 가져옵니다.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "판매 상품 정보", description = "특정 매장의 판매 물품 정보를 가져옵니다.")
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "200", description = "매장 판매 상품 조회 성공")})
    @GetMapping("/products")
    ApiResponse<List<ProductDto>> getProducts(@RequestParam long storeId);
